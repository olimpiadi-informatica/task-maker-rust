...
impl SanityChecks {
    ...
    pub fn pre_hook(&self, task: &Task, eval: &mut EvaluationData) -> Result<(), Error> {
        SanityChecks::check_task_max_score(task, eval)?;
        SanityChecks::check_att_graders(task, eval)?;
        SanityChecks::check_att_templates(task, eval)?;
        SanityChecks::check_att_sample_files(task, eval)?;
        SanityChecks::check_sol_graders(task, eval)?;
        SanityChecks::check_sol_symlink(task, eval)?;
        SanityChecks::check_sol_unique(task, eval)?;
        SanityChecks::check_statement_subtasks(task, eval)?;
        Ok(())
    }
    ...
    pub fn post_hook(&self, task: &Task, ui: &mut UIMessageSender) -> Result<(), Error> {
        SanityChecks::check_statement_valid(task, ui)?;
        SanityChecks::check_statement_git(task, ui)?;
        SanityChecks::check_broken_symlinks(task, ui)?;
        Ok(())
    }
